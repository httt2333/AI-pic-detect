...
| 2026-09-13 | `bdd930e` | Codex UX closeout window | `fix` | Closed the core review flow: truthful loading timer and hints, stable image/request lifecycle, scrollable uncapped issue list, synchronized bbox/detail actions, explicit completion, cautious dimension mapping, safer public-response validation, and Landing/demo copy alignment. Updated UX tests, browser journeys, and handoff documents. | 104 Vitest tests, 9 Playwright journeys, TypeScript, targeted ESLint, and `git diff --check` passed. | Repository-wide verify remains blocked by pre-existing formatting/lint debt; live provider acceptance is still separate. Local only. | Local only. |
